...
planetsArray.push(new Planets(1100, Math.random() * 300, 15, 0, 2 * Math.PI, 'rgba(68,28,97,0.5)', 1.9));
...
planetsArray.push(new Planets(1100, Math.random() * 300, 10, 0, 2 * Math.PI, 'rgba(255,5,75,0.5)', 1.9));
planetsArray.push(new Planets(1100, Math.random() * 300, 15, 0, 2 * Math.PI, 'rgba(183,0,42,0.5)', 1.6));
...
planetsArray.push(new Planets(1100, Math.random() * 300, 15, 0, 2 * Math.PI, 'rgba(68,28,97,0.5)', 1.6));
...
const prota = new Prota(0, 135, 30, 30, 'coral', 10)
...
enemiesArray.push(new Enemies(1100, random1, 30, 30, 'red', 2))
...
enemiesArray.push(new Enemies(100, random8, 30, 30, 'red', 2))
...
bullets.push(new Bullets(1100 + 12.5, random6 + 12.5, 5, 5, 'white', 2.5))
...
class BulletProta {
    ...
    shootHalf() {
        ...
        if (this.posX > canvas.width) {
            this.posX = prota.posX + 12.5;
            this.posY = prota.posY + 12.5;
            clearInterval(shoot)
        }
    }
    shootUp() {
        let shootUp = setInterval(() => {
            this.posX += this.speed;
            this.posY -= .20;
        }, 10)
        if (this.posX > canvas.width || this.posY < 0) {
            this.posX = prota.posX + 12.5;
            this.posY = prota.posY + 12.5;
            clearInterval(shootUp)
        }
    }
    shootDown() {
        let shootDown = setInterval(() => {
            this.posX += this.speed;
            this.posY += .20;
        }, 10)
        if (this.posX > canvas.width || this.posY > 300) {
            this.posX = prota.posX + 12.5;
            this.posY = prota.posY + 12.5;
            clearInterval(shootDown)
        }
    }
}
const bulletProtaHalf = new BulletProta(prota.posX + 12.5, prota.posY + 12.5, 5, 5, 'red', 1.5)
const bulletProtaUp = new BulletProta(prota.posX + 12.5, prota.posY + 12.5, 5, 5, 'red', 1.5)
const bulletProtaDown = new BulletProta(prota.posX + 12.5, prota.posY + 12.5, 5, 5, 'red', 1.5)
const arrayBulletsProta = [bulletProtaHalf, bulletProtaUp, bulletProtaDown];
const refresh = () => {
    canvas.width = 1000;
    canvas.height = 300;
}
    ...
    document.addEventListener('keydown', (event) => {
        ...
        if (event.key === 'w' || event.key === 'W') {
            if (prota.posY > 0) {
                ...
                arrayBulletsProta.map(index => {
                    index.posY -= prota.speed
                })
            }
        } else if (event.key === 's' || event.key === 'S') {
            if (prota.posY < canvas.height - prota.height) {
                prota.posY += prota.speed;
                for (let index of arrayBulletsProta) {
                    index.posY += prota.speed;
                }
            }
            ...
            bulletProtaHalf.shootHalf();
        }
        else if (event.key === 'q' | event.key === 'Q') {
            bulletProtaUp.shootUp();
        }
        else if (event.key === 'e' || event.key === 'E') {
            bulletProtaDown.shootDown();
        }
    })
}
const printEnemies = () => {
...
const printBullets = () => {
    for (let index = 0; index < bullets.length; index++) {
        bullets[index].paint()
        if (prota.posY >= bullets[index].posY - 50 && prota.posY <= bullets[index].posY + 50) {
            bullets[index].shoot()
        } else {
...
//Creamo la funcionde las colisiones
const colisiones = () => {
    //Colision de los enemigos con el personaje
    enemiesArray.map(indexEnemies => {//Se pone un map dentro del otro para que funcionen igual
        bullets.map(indexBullEnemies => {
            //Colision de los enemigos con el protagonista
            if (prota.posY - prota.height < indexEnemies.posY &&
                prota.posX - prota.width < indexEnemies.posX &&
                prota.posY > indexEnemies.posY &&
                prota.posX + prota.width > indexEnemies.posX
                ||
                prota.posY > indexEnemies.posY - indexEnemies.height &&
                prota.posX > indexEnemies.posX - indexEnemies.width &&
                prota.posY < indexEnemies.posY &&
                prota.posX < indexEnemies.posX + indexEnemies.width) {
                indexEnemies.posX = 1100;
                indexEnemies.posY = Math.random() * 300;
                //Colison de las balas enemigas con el personaje
            } else if (prota.posX + 30 >= indexBullEnemies.posX &&
                prota.posY >= indexBullEnemies.posY &&
                prota.posY <= indexBullEnemies.posY + 5) {
                console.log('menos una vida')
            }
            //Colision balas del protagonista con los enmigos
            arrayBulletsProta.map(indexBullet => {
                if (indexBullet.posX + 5 >= indexEnemies.posX &&
                    indexBullet.posY >= indexEnemies.posY &&
                    indexBullet.posY <= indexEnemies.posY + 30 &&
                    indexBullet.posX <= indexEnemies.posX + 30
                ){
                    indexEnemies.posX = 1100
                    indexBullet.posX = prota.posX
...
const principal = () => {
    ...
    printEnemies()
    printBullets()
    bulletProtaHalf.paint()
    bulletProtaUp.paint()
    bulletProtaDown.paint()
    colisiones()
